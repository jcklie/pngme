use std::{fs::{File, self}, path::PathBuf, io::{Read, Write}, fmt::format};
...
enum Commands {
    ...
    Encode { path: PathBuf, chunk_type: String, message: String },
...
fn read_bytes_from_file(path: &PathBuf) -> Result<Vec<u8>> {
    let mut f = File::open(&path).with_context(|| format!("File not found: '{}'", path.display()))?;
...
fn write_bytes_to_file(path: &PathBuf, bytes: &[u8]) -> Result<()> {
    ...
    file.write_all(bytes).with_context(|| format!("Error when writing to: '{}'", path.display()))?;
    ...
    match cli.command {
        ...
        Commands::Decode { path, chunk_type } => {
            ...
            if let Some(secret_chunk)  = png.chunk_by_type(&chunk_type) {
                println!("{}", String::from_utf8(secret_chunk.data().to_vec()).with_context(|| "Error converting chunk data to UTF8!")? )
            } else {
